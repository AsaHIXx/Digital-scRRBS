...
def control():
    ...
    os.system('python /home/songjia/bigdisk/xx/dingding.py -i Qual_Align_Sort')
...
def methyl_call():  
    ...
    print('a_path', a_path)
    os.system("./Methylkit_call.R -i {} -o {} > Methylkit_result".format(a_path,'./'))
    
    CpG_file = []
    for infile in sorted(glob.glob('*_CpG.txt')):
        print ("\n current file is: " + infile)
        CpG_file.append(infile)
    os.system('python ./gene_coverrage_annovar.py -s {} -p {}'.format(' '.join(CpG_file), './'))
    os.system('rm -r ./tmp')
...
def get_transrate():
    ...
    tmp_df = pd.DataFrame({'Sample': list(transrate_dict.keys()), 'Cover': list(transrate_dict.values())})
